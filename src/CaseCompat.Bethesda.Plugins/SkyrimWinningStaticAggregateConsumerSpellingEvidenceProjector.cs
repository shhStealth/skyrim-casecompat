...
// Authority state for projecting genuine winning Static consumer
...
// Pure projection of winning Static Model consumer requests into
            ...
            SkyrimWinningStaticInventoryResult inventory)
    {
        ...
        if (inventory.Winners is null)
        {
            throw new ArgumentException(
                "The winning Static inventory must retain its winner " +
                "collection.",
                nameof(inventory)
            );
        }
        ...
        foreach (
            SkyrimWinningStaticRecord? winner
            in inventory.Winners)
        {
            if (winner is null)
            {
                return Indeterminate(
                    inventory,
                    "The winning Static inventory contains a null winner."
                );
            }
                    ...
                    winner.FormKey))
            {
                return Indeterminate(
                    inventory,
                    "A winning Static has no FormKey."
                );
            }
            ...
            if (winner.ModelReferences is null)
            {
                return Indeterminate(
                    inventory,
                    $"Winning Static '{winner.FormKey}' has no retained " +
                ...
                in winner.ModelReferences)
            {
                if (reference is null)
                {
                    return Indeterminate(
                        inventory,
                        $"Winning Static '{winner.FormKey}' contains a " +
                        ...
                        out string? parseError))
                {
                    return Indeterminate(
                        inventory,
                        $"Winning Static '{winner.FormKey}' contains " +
                ...
                catch (Exception ex)
                {
                    return Indeterminate(
                        inventory,
                        $"Winning Static '{winner.FormKey}' consumer " +
