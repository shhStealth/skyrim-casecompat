...
public static class RepairPlanAggregateBatchCommand
{
    ...
    public static int Run(string[] args)
    {
            ...
            args.Length > 5)
        {
            Console.Error.WriteLine(
                "Error: repair-plan-aggregate-batch requires a Skyrim Data directory, " +
                "path-list file, batch directory, and optional manifest file name."
            );
            Console.Error.WriteLine();
            Console.Error.WriteLine(
                "Usage: casecompat repair-plan-aggregate-batch " +
        ...
        {
            Console.Error.WriteLine(
                "Repair-plan-aggregate-batch manifest file name must identify " +
        ...
        {
            Console.Error.WriteLine(
                $"Repair-plan-aggregate-batch path error: {ex.Message}"
            );
        ...
        {
            Console.Error.WriteLine(
                "Repair-plan-aggregate-batch batch directory must be outside " +
        ...
        {
            Console.Error.WriteLine(
                $"Repair-plan-aggregate-batch input error: {ex.Message}"
            );
        ...
        {
            Console.Error.WriteLine(
                "Repair-plan-aggregate-batch input contains no paths."
            );
            ...
            index++)
        {
            ...
            {
                Console.Error.WriteLine(
                    $"Repair-plan-aggregate-batch input line {index + 1} is blank."
                );
            ...
            {
                Console.Error.WriteLine(
                    $"Repair-plan-aggregate-batch input contains a duplicate path " +
            ...
            index++)
        {
            ...
            {
                Console.Error.WriteLine(
                    $"Repair-plan-aggregate-batch resolver error at line " +
                    $"{index + 1}: {ex.Message}"
                );
            ...
            {
                Console.Error.WriteLine(
                    $"Repair-plan-aggregate-batch projection error at line " +
                    $"{index + 1}: {ex.Message}"
                );
        ...
        /*
         * Aggregate mode is deliberately all-or-nothing.
         *
         * Every supplied input must produce one schema-v4 child manifest
         * before the output batch directory is even opened. These exact
         * in-memory records are the candidate set subsequently presented
         * to coverage-policy-v2.
         */
        for (
            int index = 0;
            ...
            index++)
        {
            BatchPreflightEntry entry =
                preflight[index];

            if (
                !entry.Projection.HasPlan ||
                entry.Projection.SourceSnapshot is null ||
                entry.Projection.DestinationParentSnapshot is null)
            {
                Console.Error.WriteLine(
                    "Aggregate planning requires every supplied path to " +
                    "project as an alternate-branch aggregate candidate."
                );
                Console.Error.WriteLine(
                    $"Input:      {entry.RequestedPath}"
                );
                Console.Error.WriteLine(
                ...
                creation =
                    DataRelativePathRepairPlanManifest
                        .CreateAggregateAlternateBranchFromResolution(
                            ...
                            entry.Resolution,
                            entry.Projection.SourceSnapshot,
                            entry.Projection
                                .DestinationParentSnapshot,
                            entry.Projection.Operations
                        );
            }
            catch (Exception ex)
            {
                Console.Error.WriteLine(
                    "Aggregate schema-v4 manifest creation failed for " +
                    $"{entry.RequestedPath}: {ex.Message}"
                );
                Console.Error.WriteLine(
                    "No child plan directories were published."
                );
        ...
        {
            Console.Error.WriteLine(
                $"Repair-plan-aggregate-batch directory open error: {ex.Message}"
            );
        ...
        {
            Console.Error.WriteLine(
                "Repair-plan-aggregate-batch directory could not be opened safely."
            );
            Console.Error.WriteLine(
                batchOpen.Error ??
        ...
        try
        {
            ...
            {
                Console.Error.WriteLine(
                    "Repair-plan-aggregate-batch batch directory must be empty."
                );
        ...
        {
            Console.Error.WriteLine(
                $"Repair-plan-aggregate-batch directory inspection error: " +
                $"{ex.Message}"
            );
        ...
        if (!coverageAuthorization.AllAuthorized)
        {
            Console.Error.WriteLine();
            Console.Error.WriteLine(
                "Aggregate planning rejected the complete candidate set: " +
                "coverage-policy-v2 did not prove complete recursive " +
                "physical namespace coverage."
            );

            for (
                int index = 0;
                index < coverageAuthorization.Decisions.Count;
                index++)
            {
                DataRelativePathRepairBatchCoverageDecision decision =
                    coverageAuthorization.Decisions[index];
        ...
        Console.WriteLine(
            "CaseCompat Aggregate Repair Plan Batch"
        );
        Console.WriteLine(
            "============================"
        );
            ...
            candidateIndex++)
        {
                    ...
                    .Authorized)
            {
                Console.Error.WriteLine();
                Console.Error.WriteLine(
                    "Repair-plan-aggregate-batch aggregate namespace coverage " +
            ...
            in preflight)
        {
            ...
            if (!create.Success)
            {
                Console.Error.WriteLine();
                Console.Error.WriteLine(
                    $"Repair-plan-aggregate-batch could not create child " +
                    $"directory {childName}."
                );
                Console.Error.WriteLine(
                    create.Error ??
            ...
            if (!batchSync.Success)
            {
                Console.Error.WriteLine();
                Console.Error.WriteLine(
                    $"Repair-plan-aggregate-batch could not durably publish " +
            ...
            /*
             * The complete in-memory schema-v4 candidate set has already
             * passed policy-v2 coverage, and publicationCoverage revalidated
             * that exact set immediately before this publication loop.
             *
             * Publish the exact schema-v4 manifest retained from preflight.
             * Do not reconstruct a different child record after the aggregate
             * coverage proof.
             */
            ...
            catch (Exception ex)
            {
                Console.Error.WriteLine();
                Console.Error.WriteLine(
                    $"Repair-plan-aggregate-batch could not safely open {childName} " +
            ...
            {
                /*
                 * Policy-v2 already authorized this exact schema-v4 record.
                 * Publish these exact bytes; do not reconstruct a different
                 * plan after the aggregate proof.
                 */
                expectedManifest =
                    entry.IntendedManifest!;
                ...
                catch (Exception ex)
                {
                    Console.Error.WriteLine();
                    Console.Error.WriteLine(
                        $"Repair-plan-aggregate-batch manifest write failed for " +
                ...
                if (!write.Success)
                {
                    Console.Error.WriteLine();
                    Console.Error.WriteLine(
                        $"Repair-plan-aggregate-batch manifest was not durably " +
            ...
            catch (Exception ex)
            {
                Console.Error.WriteLine();
                Console.Error.WriteLine(
                    $"Repair-plan-aggregate-batch could not reopen {childName} " +
            ...
            if (!childOpen.Success)
            {
                Console.Error.WriteLine();
                Console.Error.WriteLine(
                    $"Repair-plan-aggregate-batch could not safely reopen " +
            ...
            catch (Exception ex)
            {
                Console.Error.WriteLine();
                Console.Error.WriteLine(
                    $"Repair-plan-aggregate-batch could not inspect persisted " +
                    $"{childName}: {ex.Message}"
                );
                ...
                childInspection.ManifestRead.ManifestSha256 is null)
            {
                Console.Error.WriteLine();
                Console.Error.WriteLine(
                    $"Repair-plan-aggregate-batch could not bind persisted child " +
                    ...
                    StringComparison.Ordinal))
            {
                Console.Error.WriteLine();
                Console.Error.WriteLine(
                    $"Repair-plan-aggregate-batch persisted child {childName} " +
         ...
         * boundary.
         *
         * Persisted schema-v4 child manifests alone do not establish a
         * complete aggregate batch. Completion requires the schema-v3 batch
         * manifest to durably bind exact child membership to coverage-policy
         * version 2.
         *
         * A successful aggregate invocation has no safe-rejected subset:
         * every supplied input was required to be a schema-v4 candidate and
         * the complete set was required to pass policy-v2 coverage.
         */
        ...
        catch (Exception ex)
        {
            Console.Error.WriteLine();
            Console.Error.WriteLine(
                "Repair-plan-aggregate-batch could not create the batch " +
        ...
        catch (Exception ex)
        {
            Console.Error.WriteLine();
            Console.Error.WriteLine(
                "Repair-plan-aggregate-batch batch-completion write error: " +
                ex.Message
            );
        ...
        if (!batchWrite.Success)
        {
            Console.Error.WriteLine();
            Console.Error.WriteLine(
                "Repair-plan-aggregate-batch did not establish durable batch " +
                "completion."
            );
        ...
        catch (Exception ex)
        {
            Console.Error.WriteLine();
            Console.Error.WriteLine(
                "Repair-plan-aggregate-batch batch-completion verification read " +
                $"error: {ex.Message}"
            );
                ...
                batchVerify.Manifest))
        {
            Console.Error.WriteLine();
            Console.Error.WriteLine(
                "Repair-plan-aggregate-batch batch-completion verification failed."
            );
        ...
        Console.WriteLine();
        Console.WriteLine(
            "CaseCompat Aggregate Repair Plan Batch Summary"
        );
        Console.WriteLine(
            "===================================="
        );
        ...
        Console.WriteLine(
            "The schema-v3 batch manifest binds exact schema-v4 child " +
            "membership to aggregate namespace-coverage policy v2."
        );
        Console.WriteLine(
            "Execution support for schema-v3 / coverage-policy-v2 batches " +
            "is intentionally not enabled in this version."
        );
        Console.WriteLine();
        Console.WriteLine(
            ...
            int persistedCount)
    {
        ...
        Console.Error.WriteLine(
            "Do not blindly rerun repair-plan-aggregate-batch against this " +
            "directory."
        );
        Console.Error.WriteLine(
        ...
        Console.Error.WriteLine(
            "No repair operations were requested by repair-plan-aggregate-batch."
        );
    }
            ...
            int persistedCount)
    {
        ...
        Console.Error.WriteLine(
            "Do not blindly rerun repair-plan-aggregate-batch with the same batch " +
            "directory."
        );
        Console.Error.WriteLine(
